...
namespace sltl
{
  ...
  {
    syntax::get_current_block().add<syntax::expression_statement>(syntax::expression::make<syntax::function_call>(syntax::function_manager::get().emplace(fn)));
  }
  ...
  {
    return sltl::detail::function_traits<Fn>::return_t::proxy(syntax::expression::make<syntax::function_call>(syntax::function_manager::get().emplace(fn)));
  }
}
